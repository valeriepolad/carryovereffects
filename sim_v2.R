...
#### Code below this line needs to be included in the for loop ####

# Will be included soon--just trying to figure out best way of storing results. 
# May possibly use mulidimensional array.
...
for (i in 1:sim_replications){
  permutation_matrix[i,] <- sample(n_items)
}
...
# calculate reliability for random split halves
omega_wave1 <- as.numeric(as.character(omega(wave1,1)))
omega_wave2 <- as.numeric(as.character(omega(wave2,1)))

# difference between omega estimates (wave2 - wave1)
omega_diff <- omega_wave2[1]-omega_wave1[1]
