...
## CURRENT ISSUE -- need to be more dynamic with carryover matrix generation; currently only works with 16
n_items_vec <- 16
...
for (i in 1:length(n_items_vec)){
  ...
  for (j in 1:4){
    ...
    quarter_n_items <- n_items/4
    sigma_carryover <- matrix(rep(0,quarter_n_items*quarter_n_items), quarter_n_items, quarter_n_items)
    diag(sigma_carryover) <- carryover
    sigma_carryover <- rbind(sigma_carryover,sigma_carryover,sigma_carryover,sigma_carryover)
    sigma_carryover <- cbind(sigma_carryover,sigma_carryover,sigma_carryover,sigma_carryover)
    diag(sigma_carryover) <- 0
    
    sigma_carryover <- sigma + sigma_carryover
    ...
    for (k in 1:sim_replications){
      ...
      data_matrix_wave1 <- as.data.frame(mvrnorm(n_people, c(rep(0,n_items)), sigma))
      data_matrix_wave2 <- as.data.frame(mvrnorm(n_people, c(rep(0,n_items)), sigma_carryover))
...
end_time <- Sys.time()
  ...
  for (i in 1:length(n_items_vec)){
    ...
    for (j in 1:4){
      ...
      # create covariance matrix for carryover effects
      sigma_carryover <- matrix(rep(0,n_items*n_items),n_items,n_items)
      sigma_carryover[row(sigma_carryover) > col(sigma_carryover)] <- carryover
      ...
      for (k in 1:sim_replications){     
          ...
          wave2 <- as.data.frame(mvrnorm(n_people, c(rep(0,n_items)), sigma_carryover))
