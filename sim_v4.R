#### setup ####
# Not all of these packages are being used anymore; need to check which ones are.
library(MASS)
library(lme4)
library(psych)
library(MLmetrics)
...
all_results <- matrix(rep(0,n_items[length(n_items)]*n_items[length(n_items)]),4,n_items,dimnames = dim_names)
...
for (i in n_items){
  ...
  for (j in 1:4){
    ...
    sigma <- matrix(rep(matrix_value,n_items_this_rep*n_items_this_rep),n_items_this_rep,n_items_this_rep)
    diag(sigma) <- 1
    ...
    for (k in 1:sim_replications){
      ...
      data_matrix <- as.data.frame(mvrnorm(n_people, c(rep(0,n_items_this_rep)), sigma))
      
      # create permutation matrix for subsetting
      x <- 1:n_items_this_rep
      permutation_matrix <- do.call(rbind, rep(list(x), n_people))
      for (m in 1:n_people){
        permutation_matrix[m,] <- sample(n_items_this_rep)
      }
      
      # subsetting to construct split halves
      permutation_matrix_wave1 <- replace(permutation_matrix, permutation_matrix > (n_items_this_rep/2), TRUE)
      permutation_matrix_wave1 <- replace(permutation_matrix_wave1, permutation_matrix <= (n_items_this_rep/2), FALSE)
      permutation_matrix_wave2 <- replace(permutation_matrix, permutation_matrix <= (n_items_this_rep/2), TRUE)
      permutation_matrix_wave2 <- replace(permutation_matrix_wave2, permutation_matrix > (n_items_this_rep/2), FALSE)
