...
# value of carryover
# (0,.05,.15)
carryover_value_vec <- .05

# number of replications per cell
# needs to be increased once moved to hoffman/lab server
sim_replications <- 10
...
for (i in 1:length(n_items_vec)){
  ...
  for (j in 1:length(sigma_values)){
    ...
    omega <- switch(j,
                    "0.3", # case 1 (when sigma = .1, omega = .3)
                    "0.5", # case 2 (when sigma = .3, omega = .5)
                    "0.8", # case 3 (when sigma = .5, omega = .8)
                    "0.95", # case 4 (when sigma = .8, omega = .95)
                    print("Invalid entry."))
    ...
    # create covariance matrix for carryover effects
    
    ## remember to make dynamic later with carryover_value_vec!!! ##
    carryover_value <- .05
    
    carryover_matrix_chunk <- matrix(0,n_items,n_items)
    diag(carryover_matrix_chunk) <- carryover_sigma + carryover_value
    
    top_half <- cbind(sigma,carryover_matrix_chunk)
    bottom_half <- cbind(carryover_matrix_chunk,sigma)
    full_covariance_matrix <- rbind(top_half,bottom_half)
    
    # re-declare sigma
    sigma <- matrix(rep(matrix_value,n_items*n_items*2),n_items*2,n_items*2)
    diag(sigma) <- 1
    ...
    for (k in 1:sim_replications){
      ...
      ###data_matrix_wave1 <- as.data.frame(mvrnorm(n_people, c(rep(0,n_items*2)), sigma))
      ## generate once + then split
      data_matrix_wave2 <- as.data.frame(mvrnorm(n_people, c(rep(0,n_items*2)), full_covariance_matrix))
      ...
      wave1 <- data_matrix_wave1*permutation_matrix_wave1
      wave2 <- data_matrix_wave2*permutation_matrix_wave2
...
end_time <- Sys.time()
  ...
  for (i in 1:length(n_items_vec)){
    ...
    for (j in 1:length(sigma_values)){
      ...
      # create covariance matrix for carryover effects
      carryover_sigma <- matrix_value/2
      ...
      for (k in 1:sim_replications){     
          # Needs to be sample size (n_people) throughout section
          wave1 <- as.data.frame(mvrnorm(n_people, c(rep(0,n_items*2)), sigma))
          wave2 <- as.data.frame(mvrnorm(n_people, c(rep(0,n_items*2)), full_covariance_matrix))
...
# can carryover be introduced here?
# https://www.rdocumentation.org/packages/rockchalk/versions/1.8.144/topics/mvrnorma